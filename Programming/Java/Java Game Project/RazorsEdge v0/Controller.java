...
public class Controller extends Display
{
    ...
    public Controller(Player[] pList, int mapSizeIn, Tile[][] ter, int sightRangeIn, Importer imp)
    {
        importer = imp;
        int[] speed = new int[pList.length];
        for (int i = 0 ; i < pList.length ; i++)
        {
            speed[i] = (int)(pList[i].getSpeed());
        }
        playerList = new Player[pList.length];
        ...
        for (int i = 0; i < pList.length; i++)
        {
            ...
            for (int k = 0; k < pList.length; k++)
            {
                ...
                //System.out.println(highestN+" versus "+pList[k].getSpeed());
                if (speed[k]>highestN)
                {
                    highestN = (int)pList[k].getSpeed();
                    rIndex = k;
                }
            }            
            playerList[i] = pList[rIndex];
            
            speed[rIndex] = -999;
        }

        System.out.println(sightRangeIn);
        objectArray = new Object[mapSizeIn][mapSizeIn];

        sightRange = sightRangeIn;

        globalScaleIndex = 4;
        localScaleIndex = 1;
        globalWidth = mapSizeIn;
        globalHeight = mapSizeIn;
        localWidth = (sightRange/5)+1;
        localHeight = (sightRange/5)+1;
        mapSize = mapSizeIn;
        terrain = ter;
        this.isSightRange();
    }
    ...
    public void onMousePress(MouseEvent m)
    {
        ...
        {
            InventoryDisplay inv = new InventoryDisplay(playerList);
            inv.launch();
        }
    ...
    public static void demo()
    {
        ...
        for (int i = 0; i <playerList.length; i++)
        {
            Player tmpPlayer = new Player(i,1);
            for (int k = 0; k < 5; k++)
            {
        ...
        Controller cO = new Controller(playerList,2400,terrain,250,importer);
        cO.launch();
    }
