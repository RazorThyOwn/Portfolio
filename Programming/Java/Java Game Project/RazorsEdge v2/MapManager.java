...
public class MapManager extends Display
{
    ...
    public void onMousePress(MouseEvent m)
    {
        for (int i = 0; i < btPlayerList.length; i++)
        {
            ...
            {
                Player playr = playerList[i];
                double sight = terrain[playr.getXPos()][playr.getYPos()].getSight();
                int vision = (int)( ((double)playerWidth)*sight ) +1;
                int xCord = playerList[i].getXPos() - playerWidth/2; 
                int yCord = playerList[i].getYPos() - playerHeight/2;
                Grid playerGrid = new Grid(vision,vision,0,localScaleIndex,xCord,yCord,objectArray,terrain);
                playerGrid.launch();
            }
        }
        ...
        if (globalButton.press(m.getX(),m.getY()))
        {
            ...
            Grid globalGrid = new Grid(globalWidth/intAr[globalScaleIndex],globalHeight/intAr[globalScaleIndex],-1,intAr[globalScaleIndex],0,0,objectArray,terrain);
            globalGrid.launch();
        }
    }
